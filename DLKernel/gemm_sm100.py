# Copyright (c) 2025-2026, DLKernel team.
...
from DLKernel.dsl.smem_struct import Reserved, partitioned_struct
from DLKernel.tile_scheduler import (
    TileSchedulerOptions,
    ag_wait_m_tile,
)
from DLKernel.varlen_utils import VarlenArguments, VarlenManager
from DLKernel.gemm_base import GemmTmaBase, NamedBarrierGemm
from DLKernel.gemm_config import SplitKMode
from DLKernel import layout_utils
from DLKernel import pipeline_checks
import DLKernel.copy_utils as copy_utils
import DLKernel.sm100_utils as dlkernel_sm100_utils
from DLKernel.epilogue.quantize_out import active_row_sfd_reqs
from DLKernel.layout_utils import tile_atom_to_shape_SF_strided
...
#     overhead model in DLKernel/distributed/all_gather_gemm.py's docstring).
...
from DLKernel.gemm_base import reinterpret_packed_fp6 as _reinterpret_packed_fp6  # noqa: E402
...
class GemmSm100(GemmTmaBase):
    ...
    Warp roles and pipeline schedule (arrive counts validated by DLKernel.pipeline_checks
    ...
    def _setup_attributes(self, epilogue_args: EpilogueArguments, varlen_args: VarlenArguments):
        ...
        if const_expr(not self.blockscaled):
            ...
            # DLKernel's wrapper, not the DSL helper: both-fp4 pairs always run
            # the single kind::mxf4nvf4 atom with the format's scale config.
            self.tiled_mma = dlkernel_sm100_utils.make_blockscaled_trivial_tiled_mma(
                self.a_mma_dtype,
                self.b_mma_dtype,
                self.a_major_mode,
                self.b_major_mode,
        ...
        if const_expr(self.gather_A):
            if const_expr(self.use_tma_gather):
                self.a_smem_load_layout_staged = dlkernel_sm100_utils.make_smem_layout_tma_gather_a(
                    self.tiled_mma, self.mma_tiler, self.a_dtype, self.ab_stage
                )
            else:
                self.a_smem_load_layout_staged = dlkernel_sm100_utils.make_smem_layout_cpasync_a(
                    self.tiled_mma, self.mma_tiler, self.a_dtype, self.ab_stage
                )
    ...
    def __call__(
        ...
        if const_expr(not self.gather_A):
            ...
            tma_smem_layout = dlkernel_sm100_utils.make_smem_layout_atom_tma_gather_a(
        ...
        if warp_idx == self.epi_load_warp_id:
            ...
            if const_expr(has_epi_load):
                ...
                while work_tile.is_valid_tile:
                    ...
                    # folds at trace time (DLKernel.dsl.mixed_constexpr_if), so
    ...
    def _blockscaled_mma_inst_k(a_copy_dtype, b_copy_dtype) -> int:
        ...
        32. Mirrors DLKernel.blockscaled.operand.mma_kind_for_pair; keep the two in
    ...
    def is_valid_dtypes_and_scale_factor_vec_size(
        ...
        a separate, wider set - see DLKernel.blockscaled.operand.mma_kind_for_pair;
    ...
    def can_implement(
        ...
        :class:`DLKernel.blockscaled.operand.BlockScaledFormat` descriptors,
