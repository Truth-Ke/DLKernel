...
lives in :mod:`DLKernel.cache` (the package init).

When an async compile pool is active (see :mod:`DLKernel.cache.async_compile`),
...
# `DLKernel.cache` (the package itself) holds the mutable runtime flags as a
...
import DLKernel.cache as _state  # noqa: E402  (intentional partial-import; see __init__.py)
...
def get_cache_path() -> Path:
    if _state.CACHE_DIR is not None:
        ...
        cache_dir = Path(tempfile.gettempdir()) / getuser() / "dlkernel_cache"
...
def _compute_source_fingerprint() -> str:
    ...
    import DLKernel as _pkg

    _hash_source_dir(h, Path(_pkg.__file__).resolve().parent)
    ...
    * **Slow path (cold cache).** The actual ``fn(*args, **kwargs)`` compile
    ...
    def wrapper(*args, **kwargs):
        ...
        def _quarantine_corrupt(exc: Exception) -> None:
            ...
            print(
                f"DLKernel cache: corrupt cached object for key {sha} "
        ...
        from DLKernel.cache import async_compile as _async
        ...
        if pool is not None:
            ...
            if state == "new":
                ...
                warnings.warn(
                    f"DLKernel cache: async compile failed for {fn.__qualname__} "
        ...
        try:
            ...
            warnings.warn(
                f"DLKernel cache: lock timeout for key {sha}: {e}; "
        ...
        try:
            ...
            try:
                ...
                warnings.warn(
                    f"DLKernel cache: export failed for key {sha}: {e} "
