...
``CUDA_VISIBLE_DEVICES=""`` + ``DLKERNEL_ARCH``/``CUTE_DSL_ARCH`` overrides so
...
# Pin both arch overrides BEFORE importing DLKernel: import-time code paths
# (e.g. rmsnorm_config._detect_arch_major) consult DLKERNEL_ARCH via
...
#
# NOTE: by the time this module body runs, its parent packages (``DLKernel``,
# ``DLKernel.cache``) have already been imported — and ``DLKernel/__init__`` pulls
...
# (DLKERNEL_ARCH dispatch, the gpu-blind smem-capacity shim).
#
# The two overrides are pinned independently: DLKERNEL_ARCH (dispatch) is
...
# DLKERNEL_ARCH — because the main process loads the .o on the physical GPU
...
try:
    ...
    os.environ.setdefault("DLKERNEL_ARCH", f"{major}{minor}")
...
import DLKernel.cache  # noqa: F401, E402  (pulls torch, cutlass.cute, tvm_ffi)
...
from DLKernel.cache.async_compile import _pin_dsl_arch  # noqa: E402
