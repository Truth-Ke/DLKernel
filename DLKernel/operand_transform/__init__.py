...
``transform_a=`` to :func:`DLKernel.gemm_runtime.host.build_gemm_epi_plan` — or
...
every epilogue variant (see DLKernel.operand_transform.host for the picklable
reference + the W4 blob/strip geometry). ``DLKernel.gemm_w4.gemm_w4a16`` is
thin sugar over that path.

Layering note: the fn frontend (a_transform / w4_transform) sits ABOVE the
host layer (it imports DLKernel.gemm_runtime.identity for the semantic keyer
...
from DLKernel.operand_transform.transform import (  # noqa: F401
    AuxKTileStrip,
    AuxOperandA,
    TransformA,
    TransformADropout,
...
def __getattr__(name):
    if name in _FRONTEND:
        from DLKernel.operand_transform import frontend

        return getattr(frontend, name)
    if name in _HOST:
        from DLKernel.operand_transform import host
