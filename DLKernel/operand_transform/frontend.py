...
* DROPOUT (``dropout_a(p)``): not a fn — a dedicated mask-only transform
  ...
  :class:`~DLKernel.operand_transform.transform.TransformADropout`). The
...
* PACKED decodes (``packed=PackedInput(...)``): the fn IS the decode — the
  :meth:`~DLKernel.operand_transform.formats.DecodeFormat.decode_k16` body,
...
``GemmSm90(transform_a=mod)``. ``__dlkernel_semantic_key__`` fail-closed
...
records the single ``dlkernel::gemm_epi`` custom op: mods cross by
...
ride the op's input list. See DLKernel.gemm_runtime.torch_op.
"""
...
from DLKernel.operand_transform.formats import DecodeFormat, decode_format
...
from DLKernel.operand_transform.kinds import ARG_KINDS
from DLKernel.operand_transform.transform import TransformAValue, TransformAW4
...
class PackedInput:
    ...
    Field meanings match :class:`~DLKernel.operand_transform.formats.DecodeFormat`
...
class TransformModBase:
    ...
    (:func:`DLKernel.operand_transform.host.as_transform_mod`) and call these
    ...
    def default_config(self, A, B):
        ...
        from DLKernel.operand_transform.host import pick_w4_cfg, pick_w4a8_cfg
    ...
    def padded_n(self, B):
        ...
        if self.owned_fmt is None:
            return None
        from DLKernel.operand_transform.host import w4_padded_n
    ...
    def resolve_operands(self, A, B, transform_sf, tile_m, tile_k) -> ResolvedOperands:
        ...
        from DLKernel.operand_transform.host import w4_operand_views
    ...
    def fake_bundle(self, mA_fake, a_dtype, tile_m, tile_k):
        ...
        from DLKernel.operand_transform.host import transform_a_fake_operand
    ...
    def fake_operands(self, n_full, k, tile_m):
        ...
        from DLKernel.operand_transform.host import w4_fake_operands
...
class ATransformMod(TransformModBase):
    ...
    def __init__(self, fn, vec_size, packed, consts=None, regs=None, args=None):
        ...
        self._fmt = None
        self.semantic_digest = _digest(self.__dlkernel_semantic_key__())
        TORCH_OP_TRANSFORM_MODS[self.semantic_digest] = self  # dlkernel::gemm_epi resolution
    ...
    def _module_locator(self):
        ...
        process (the dlkernel::gemm_epi custom op re-resolves it that way when a
...
class DropoutAMod(TransformModBase):
    ...
    def __call__(self, gemm):
        from DLKernel.operand_transform.transform import TransformADropout
