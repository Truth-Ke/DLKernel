...
generic GEMM host layer (:mod:`DLKernel.gemm_runtime.host`), mirroring what EpiOps'
host hooks do for epilogues:

* :func:`as_transform_mod` — normalize any handle to a TransformModBase
  mod (refs/registries/payloads live in :mod:`DLKernel.gemm_runtime.identity`,
...
from DLKernel.operand_transform.formats import DecodeFormat, W4_FORMATS
...
def _named_w4_mod(name: str):
    from DLKernel.operand_transform.frontend import w4_transform
...
def as_transform_mod(handle):
    ...
    if isinstance(handle, DecodeFormat):
        ...
        if mod is None:
            from DLKernel.operand_transform.frontend import PackedFormatMod
...
# ---- DLKernel.gemm_w4's explicit-tile surface) --------------------------------
...
def _sms_or_default(device_index: int = 0, default: int = 170) -> int:
    ...
    (``DLKERNEL_ARCH`` + ``CUDA_VISIBLE_DEVICES=""``) and trace-time meta/fake
    tensors have no device, and querying eagerly raises
    ``CUDA_ERROR_NOT_INITIALIZED`` there. Same fallback convention as
    :func:`DLKernel.gemm_heuristic._device_physics` (try, then a per-arch
    ...
    try:
        from DLKernel.cute_dsl_utils import get_device_multiprocessor_count
