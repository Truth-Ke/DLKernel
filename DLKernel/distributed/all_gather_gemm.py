...
AB-load warp before a tile's first TMA — both in shared code, so ANY DLKernel
...
GEMM(s) in a context::
    ...
    with runner.gather(a_shard) as (a_full, ag_args):
        d = gemm(a_full, b, ..., ag_args=ag_args)  # any DLKernel GEMM(s)
...
from DLKernel.gemm import AllGatherArguments
...
class AllGatherRunner:
    ...
    - forwards ``ag_args`` (a :class:`DLKernel.gemm.AllGatherArguments`, or
      None on the world_size == 1 fast path) verbatim to a persistent dense
      DLKernel GEMM — ``ag_args`` is a universal kwarg, any variant works
    ...
    def gather(self, a_shard: torch.Tensor):
        ...
        world_size == 1 fast path) to the DLKernel GEMM(s) — ag_args is a
...
class BlockScaledAllGatherRunner(AllGatherRunner):
    ...
    :class:`~DLKernel.blockscaled.operand.BlockScaledOperand` X engages the
    ...
    the parent. The operand is DLKernel's canonical (qdata, scale, format)
