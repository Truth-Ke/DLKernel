...
from DLKernel.gemm_tvm_ffi_utils import div_for_dtype, make_scheduler_args
...
from DLKernel.blockscaled.quantize import (  # noqa: F401  (pure-torch helpers re-exported)
    FP4_E2M1FN_VALUES,
    QUANTIZERS,
    _COMPILE_KW,
...
# descriptors in DLKernel.blockscaled.operand are the single source of truth.
BLOCKSCALED_FORMATS = {
...
def blockscaled_quantize(
    ...
    Returns ``(q, sf)`` quantizer outputs, NOT a GEMM operand: DLKernel GEMMs take
...
def blockscaled_quantize_dim0(
    ...
    """
    from DLKernel.blockscaled.quantize import to_mx_dim0_compiled
...
def create_blockscaled_operand_quantized(
    ...
    *,
    ...
    q_mkl: (mn, k, l) operand tensor in the layout the DLKernel GEMMs consume
    ...
    scale_contig: (l, rm, rk, 32, 4, 4) contiguous scale storage. Each inner
           ...
           cuBLAS `to_blocked`. Pass directly to the DLKernel GEMM, or use
...
def create_blockscaled_varlen_k_operands(
    ...
    *,
    ...
    from DLKernel.blockscaled.quantize import to_mx_compiled
...
def compile_blockscaled_gemm_tvm_ffi(
    ...
    *,
    ...
    the DLKernel.gemm_interface layer's job; unwrap before calling this.
    ...
    partials workspace per call (mirroring DLKernel.gemm.gemm) and threads them through.
    ...
    assert not isinstance(mA, BlockScaledOperand) and not isinstance(mB, BlockScaledOperand), (
        ...
        "(use .qdata / .scale) or call DLKernel.gemm"
    )
    ...
    from DLKernel.gemm_tvm_ffi_utils import make_fake_varlen_args
    ...
    if varlen_m:
        ...
        # the public API enforces this (see DLKernel/gemm.py) for all dtypes.
        fake_mA = fake_tensor(
            ab_dtype,
            (total_m_sym, k_sym),
    ...
    if split_k_compile:
        ...
        def run(a, b, d, sfa, sfb):
            ...
            # mirroring DLKernel.gemm.gemm. d is (m, n, l) here.
            num_l = d.shape[2]
            ...
            compiled(
                ...
                # not rotate them; pass kernel-order views (same as DLKernel.gemm.gemm).
                sem.permute(1, 2, 0),
                ws.permute(3, 1, 2, 0),
                VarlenArguments(),
            )
