...
- ``BlockScaledOperand`` is a plain frozen dataclass holding ``qdata`` (quantized
  ...
  DLKernel GEMM entry points are its only consumers, and the simplest container that
...
by ``DLKernel.gemm_interface`` at module level and by the kernel-layer modules
lazily.
"""
...
class BlockScaledFormat:
    ...
    def from_cutlass_dtypes(cls, ab_dtype, sf_dtype, sf_vec_size: int) -> "BlockScaledFormat":
        ...
        from DLKernel.cute_dsl_utils import torch2cute_dtype_map  # lazy: needs the DSL
...
class BlockScaledOperand:
    """Block-scaled quantized operand for DLKernel GEMMs.
    ...
    *logical* view (unpacked shape, original high-precision dtype); storage truth
    ...
    surface is DLKernel.gemm* plus the explicit methods here.
    ...
    def to_packed(self) -> "BlockScaledOperand":
        ...
        if not fmt.is_byte_container:
            ...
            return self
        from DLKernel.blockscaled.quantize import _pack_uint4, pack_uint6
...
_pytree.register_pytree_node(
    ...
    _flatten,
    _unflatten,
    serialized_type_name="DLKernel.blockscaled.operand.BlockScaledOperand",
