...
contract (DLKernel.epilogue.frontend) on top of the EpiOp library (DLKernel.epilogue.ops).
...
Sections:
  ...
  * reusable domain resources live in ``DLKernel.epilogue``: rotary table loads
...
from DLKernel.activation import (
    act_fn_map,
    dact_fn_map,
    dgate_fn_map,
    dgelu_tanh_approx,
...
from DLKernel.epilogue.head_rmsnorm import HeadRstd
from DLKernel.epilogue.quantize_out import BlockScaleFactorStore
from DLKernel.epilogue.rotary import rotary_cos_sin_load
from DLKernel.epilogue.math import pack, pexp, unpack
...
def _gen_epi_fn(fname, tag, params, body, ns):
    ...
    src = "\n".join(lines)
    code = compile(src, f"<DLKernel-epilogue:{tag}>", "exec")
    ns = {**ns, "unpack": unpack, "pack": pack, "__name__": "DLKernel.epilogue.library_generated"}
    exec(code, ns)
    return ns[fname]
