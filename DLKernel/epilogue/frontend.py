...
Layer map
---------
* DLKernel.epilogue.ops        — EpiOps: per-tensor RESOURCE lifecycle (smem, TMA,
                              fragments, flushes: begin/begin_loop/end_loop/
                              end), host schema hooks (host_arg_key/
                              host_fake_arg/host_call_arg), and VALUE PORTS
                              (below).
* DLKernel.gemm_runtime.identity — the LEAF: semantic fingerprints, digest
                              registries, picklable class/mod refs.
* DLKernel.gemm_runtime.host   — generic host layer: one jit-cached compile fn
...
* this module               — the fn contract + kernel-class minting. A mod is
                              ...
                              live in DLKernel.epilogue.visit), so hand-written
                              ...
                              DLKernel.epilogue.math.
* DLKernel.epilogue.library    — the library of ready mods and reusable ops.
...
DLKernel.activation, whose functions compose directly; F2 arithmetic lowers to
...
implementing ``__dlkernel_semantic_key__(self) -> object`` are supported;
...
:class:`~DLKernel.gemm_runtime.identity.GemmClassRef` recipe that re-mints the class at the
...
from DLKernel.cute_dsl_utils import get_device_capacity, mlir_namedtuple, torch2cute_dtype_map
from DLKernel.epilogue.ops import (
    ColVecLoad,
    EpiOp,
    RowVecLoad,
    Scalar,
...
from DLKernel.gemm_runtime.identity import (
    GemmClassRef,
    TORCH_OP_EPI_MODS,
...
from DLKernel.gemm_tvm_ffi_utils import tensor_key
from DLKernel.gemm_sm80 import GemmSm80
from DLKernel.gemm_sm90 import GemmSm90
from DLKernel.gemm_sm100 import GemmSm100
from DLKernel.gemm_sm120 import GemmSm120
...
class EpiMod:
    ...
    def __init__(
        ...
        TORCH_OP_EPI_MODS[self.semantic_digest] = self  # dlkernel::gemm_epi resolution
    ...
    def _class_ref(self, mint_key):
        ...
        if locator is None:
            ...
            # by value (GemmClassRef.__dlkernel_pool_payload__).
    ...
    def gemm_tuned(self, A, B, D, C=None, *, epi_args: dict, b_kn: bool = False):
        """Autotuned gemm(): config-space sweep via DLKernel.gemm_runtime.autotune (lazy
        import — this module sits below gemm_config in the import graph).
        Returns TunedModGemm(plan, config, sinks); see tuned_mod_gemm."""
        from DLKernel.gemm_runtime.autotune import tuned_mod_gemm
    ...
    def gemm(
        ...
        *,
        ...
        concat_layout=None,
        SFA=None,  # blockscaled scale factors, see DLKernel.gemm
        SFB=None,
        ...
        split_k: int = 1,  # K-dim split factor (SERIAL/PARALLEL only; see DLKernel.gemm)
        ...
        ag_args=None,  # AllGather+GEMM flags contract (see DLKernel/distributed/)
        ...
        split_k_buffers=None,  # (sem, ws) from DLKernel.gemm._split_k_buffers: reuse across
        ...
        if blockscaled:
            from DLKernel.gemm_tvm_ffi_utils import (
    ...
    # eager ``__call__`` (autotunes via DLKernel.gemm_runtime.autotune, allocates missing
    ...
    def __call__(
        ...
        **operands,  # epilogue operand tensors/scalars by fn-parameter name
        ...
        DLKernel.gemm_runtime.autotune on first sight of a metadata class), allocate
        ...
        The tuned path covers the non-SR surface (see DLKernel.gemm_runtime.autotune,
        ...
        Under torch.compile the call records the single ``dlkernel::gemm_epi``
        custom op (see DLKernel.gemm_runtime.torch_op); with reduce sinks the config is
        ...
        if needs_transform_operands:
            from DLKernel.operand_transform.transform import TransformAOperand
        ...
        if torch.compiler.is_compiling():
            ...
            from DLKernel.gemm_runtime.torch_op import compile_call
