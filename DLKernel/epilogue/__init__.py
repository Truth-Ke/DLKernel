...
* :mod:`DLKernel.epilogue.ops` — the EpiOp vocabulary: per-tensor resource
  lifecycle, value ports, host schema, cache identity. Writing a new op here
  (or in a domain module) is the extension path.
* :mod:`DLKernel.epilogue.mixin` — ComposableEpiMixin: composes ``_epi_ops``
  into the kernel epilogue hooks (hand-written mixins subclass this).
* :mod:`DLKernel.epilogue.frontend` — ``@gemm_epilogue``: author an epilogue as
  a plain fn over the accumulator; EpiMod mints kernel classes and owns the
  torch-facing call surfaces. :mod:`DLKernel.epilogue.visit` is its device half.
* :mod:`DLKernel.epilogue.math` — the value vocabulary (Pair/F2/pexp...).
* :mod:`DLKernel.epilogue.library` — the library of ready mods; domain content
...
from DLKernel.epilogue.math import (  # noqa: F401
    F2,
    F16Lanes,
    Pair,
    pack,
...
def __getattr__(name):
    if name in _FRONTEND:
        from DLKernel.epilogue import frontend
