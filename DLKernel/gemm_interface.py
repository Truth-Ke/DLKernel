...
# module sits above DLKernel.gemm in the import graph (importing it from here would cycle).
...
from DLKernel.gemm import gemm as gemm_dispatch, run_gemm_plan
from DLKernel.gemm_iface import (
    IfacePlan,
    VariantSpec,
    alloc_outputs,
    make_iface_plan,
...
#
# Blockscaled A / B operands are :class:`DLKernel.blockscaled.operand.BlockScaledOperand`
...
def _unpack_operand(X) -> _Operand:
    ...
    if isinstance(X, (tuple, list)):
        raise TypeError(
            ...
            "DLKernel.blockscaled.BlockScaledOperand.from_parts(data, scale_factor, format)"
        )
    return _Operand(X)
...
def _reserve_blockscaled_out(out_dtype) -> None:
    ...
    SF-generation epilogue (DLKernel.epilogue.quantize_out); the other entry points
    still reject it here."""
    if _resolve_blockscaled_out(out_dtype) is not None:
        raise NotImplementedError(
            f"blockscaled output (out_dtype={out_dtype}) is only supported by DLKernel.gemm "
...
def nvmmh_config(A, B, device_capacity):
    ...
    """
    try:
        from DLKernel.nvmmh_heuristic import nvmmh_default_config
...
# DLKernel.epilogue.library.linear_act_mod owns canonicalization, plan caching, and
...
def _gemm_act_call(
    ...
    *,
    ...
    bs_format_a: Optional[str] = None,  # BlockScaledFormat names (see DLKernel.gemm.gemm)
    bs_format_b: Optional[str] = None,
...
# Ported to the epilogue-object surface (see the gemm_rms note below):
# DLKernel.epilogue.library.dact_mod / dgated_mod own canonicalization, plan caching,
...
def _gemm_dact_call(
    ...
    *,
    ...
    from DLKernel.epilogue.library import dact_mod, dgated_mod
...
def gemm_blockscaled_ref(
    ...
    from DLKernel.blockscaled.utils import dequant_operand, unpack_scale_blocked_to_2d
...
@torch.library.custom_op("dlkernel::gemm_add", mutates_args=(), device_types="cuda")
...
@torch.library.custom_op("dlkernel::gemm", mutates_args=(), device_types="cuda")
...
@torch.library.register_fake("dlkernel::gemm_dgated_out")
...
@torch.library.register_fake("dlkernel::gemm_dact_out")
...
## ── gemm_rms ────────────────────────────────────────────────────────────────
# Ported to the epilogue-object surface: DLKernel.epilogue.library.sq_reduce_mod owns
...
def _gemm_rms_call(
    ...
    *,
    ...
    from DLKernel.epilogue.library import sq_reduce_mod
...
@torch.library.register_fake("dlkernel::gemm_rms_out")
...
def gemm_rms(
    ...
    if torch.compiler.is_compiling():
        ...
        # sinks, the generic dlkernel::gemm_epi path would pin the config so the
...
# DLKernel.epilogue.library.norm_act_mod owns canonicalization, plan caching, and tuning
...
#         torch.ops.dlkernel.gemm_add_out.default:
#         InplaceableOp(torch.ops.dlkernel.gemm_add_inplace.default, mutated_arg=2)
#     })
# except ImportError:
#     pass
