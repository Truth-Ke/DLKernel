...
Mirrors :mod:`DLKernel.gemm_config`: frozen dataclasses that capture the launch
...
def _max_dynamic_smem_bytes() -> int:
    ...
    (honors the ``DLKERNEL_ARCH`` override) and looks the capacity up in the
    ...
    if os.environ.get("DLKERNEL_ARCH") is None and not torch.cuda.is_available():
        return 0
...
def _detect_arch_major() -> int:
    ...
    Honors the ``DLKERNEL_ARCH`` override (via ``get_device_capacity``) so
    GPU-blind processes — compile-pool workers, CPU-only boxes — never
    initialize CUDA here. This function runs at ``import DLKernel`` time (the
