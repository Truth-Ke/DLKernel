...
from DLKernel.dsl import cute_op

from DLKernel.pipeline import make_pipeline_state, PipelineStasAsync
from DLKernel.reduce import row_reduce
...
def rmsnorm_bwd(
    ...
    if not per_head and weight is not None and not has_bias and x.numel() > 0:
        ...
        if dresidual_out is not None and dresidual_out.dtype != x.dtype:
            dx, dw_partial, dresidual = torch.ops.dlkernel.rmsnorm_bwd_f_dual(
                x, weight, dout, rstd, dresidual_out, sm_count, weight_offset=weight_offset
            )
        else:
            dx, dw_partial = torch.ops.dlkernel.rmsnorm_bwd_f(
                x, weight, dout, rstd, dresidual_out, sm_count, weight_offset=weight_offset
            )
            dresidual = None
...
@torch.library.custom_op("dlkernel::rmsnorm_bwd_f", mutates_args=(), device_types="cuda")
def _rmsnorm_bwd_f(
    x: Tensor,
    weight: Tensor,
    dout: Tensor,
...
@torch.library.custom_op("dlkernel::rmsnorm_bwd_f_dual", mutates_args=(), device_types="cuda")
...
class DLKernelRMSNorm(torch.nn.RMSNorm):
    ...
    the DLKernel.rmsnorm implementation under the hood.
