...
"""Reusable pytest plugin for DLKernel async kernel compilation.
...
see :mod:`DLKernel.cache.async_compile`), the test is deferred while other tests
...
def _install_getfuncargnames_cache() -> None:
    ...
    * Honor ``DLKERNEL_PYTEST_NO_GETFUNCARGNAMES_CACHE=1`` to opt out at runtime.
    ...
    if os.environ.get("DLKERNEL_PYTEST_NO_GETFUNCARGNAMES_CACHE"):
        return
    ...
    if orig is None or getattr(_fixtures, "getfuncargnames", None) is not orig:
        warnings.warn(
            "DLKernel.testing.pytest_plugin: skipping getfuncargnames cache; "
    ...
    try:
        ...
        warnings.warn(
            f"DLKernel.testing.pytest_plugin: skipping getfuncargnames cache; "
...
def pytest_configure(config):
    ...
    if jobs is not None:
        ...
        if not is_xdist_master:
            ...
            from DLKernel.cache.async_compile import activate
            ...
            if worker is not None:
                config.pluginmanager.register(_XdistWorkerDefer(pool), "DLKernel-xdist-defer")
            else:
                config.pluginmanager.register(_SingleProcDeferLoop(pool), "DLKernel-defer-loop")
...
def pytest_unconfigure(config):
    ...
    from DLKernel.cache.async_compile import get_active_pool, deactivate
    ...
    if pool is not None:
        ...
        defer_plugin = config.pluginmanager.get_plugin(
            "DLKernel-defer-loop"
        ) or config.pluginmanager.get_plugin("DLKernel-xdist-defer")
...
def _defer_if_compile_pending(item, outcome, force_pass: bool) -> bool:
    ...
    if not issubclass(outcome.excinfo[0], CompilePending):
        return False
    item._dlkernel_pending_sha = outcome.excinfo[1].sha
    if force_pass:
...
def _run_protocol(item, nextitem, force_sync: bool):
    ...
    from DLKernel.cache.async_compile import suppress_pool
    ...
    item._dlkernel_pending_sha = None
    if force_sync:
        with suppress_pool():
            reports = runtestprotocol(item, nextitem=nextitem, log=False)
    else:
        reports = runtestprotocol(item, nextitem=nextitem, log=False)
    return getattr(item, "_dlkernel_pending_sha", None), reports
...
class _SingleProcDeferLoop:
    ...
    def pytest_runtestloop(self, session):
        ...
        while queue:
            item = queue.popleft()
            awaiting = getattr(item, "_dlkernel_await_sha", None)
            ...
            if pending_sha:
                item._dlkernel_await_sha = pending_sha
