...
out, VEC16_UE4M3 for nvfp4 out) — the same SF-generation epilogue DLKernel's
...
Layout mapping (everything row-major on the torch side):
    ...
    to DLKernel's blocked (rm, rk, 32, 4, 4) atom, flattened (see
    scale_blocked_for_cublas) — A-slot scale = SFB, B-slot scale = SFA, and the
    D-out scales come back directly comparable with DLKernel's SFD.
"""
...
class CublasLtQuantOutGemm:
    ...
    row-major in the same value dtype; SFD comes back in DLKernel's blocked
