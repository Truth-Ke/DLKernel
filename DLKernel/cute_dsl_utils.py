...
def _parse_arch_str(arch_str: str) -> Tuple[int, int]:
    ...
    if not match:
        raise ValueError(f"Invalid DLKERNEL_ARCH format: {arch_str!r} (expected e.g. '90', 'sm_90')")
...
def _get_device_capacity_cached(device: torch.device = None) -> Tuple[int, int]:
    ...
    Override with DLKERNEL_ARCH (e.g. 'sm_90' or '90') for CPU-only compilation
    without a GPU present.
    """
    arch_override = os.environ.get("DLKERNEL_ARCH")
...
def get_compile_target_capacity() -> Tuple[int, int]:
    ...
    and honors ``DLKERNEL_ARCH``. This helper answers "which instructions may the
    traced code emit" — a property of the actual compile target, and the two
    diverge on the CI proxy legs (``DLKERNEL_ARCH=120`` on an H100 still compiles
    ...
    ``DLKERNEL_ARCH`` (CPU-only cross-compile box, where dispatch arch is the
    ...
    raise RuntimeError(
        ...
        "CUTE_DSL_ARCH nor DLKERNEL_ARCH is set"
    )
