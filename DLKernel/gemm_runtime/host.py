...
from DLKernel.compile_utils import make_fake_tensor
from DLKernel.cute_dsl_utils import get_max_active_clusters, torch2cute_dtype_map
from DLKernel.gemm_config import SplitKMode, cta_tile_shape_m
from DLKernel.epilogue.ops import EpiOp
...
def _compile_gemm_epi(
    ...
    if split_k > 1 and split_k_mode != SplitKMode.SEPARATE:
        # Mirrors DLKernel.gemm: (ntile_m, ntile_n, L) Int32 per-tile flag and
...
def build_gemm_epi_plan(
    ...
    *,
    ...
    has_ag=False,  # AllGather+GEMM (see DLKernel/distributed/): dense persistent only
    split_k=1,
    ...
    *,
    ...
    if plan.split_k > 1:
        if split_k_buffers is None:
            # Fresh per-call buffers (mirrors DLKernel.gemm.run_gemm_plan); lazy import —
            # DLKernel.gemm sits above this module in the import graph.
            from DLKernel.gemm import _split_k_buffers
