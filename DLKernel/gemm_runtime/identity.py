...
nothing from DLKernel at module level; anyone may import it):
...
* ``TORCH_OP_EPI_MODS`` / ``TORCH_OP_TRANSFORM_MODS`` — digest -> mod for
  ``dlkernel::gemm_epi`` resolution (written at mod CONSTRUCTION, never inside
...
def semantic_value_key(value, seen):
    ...
    ``__dlkernel_semantic_key__(self) -> object`` (recursed through this same
    ...
    if value is None or isinstance(value, (bool, int, float, complex, str, bytes)):
        return value
    qsk = getattr(type(value), "__dlkernel_semantic_key__", None)
    if qsk is not None:
        marker = ("id", id(value))
        if marker in seen:
    ...
    raise TypeError(
        ...
        "__dlkernel_semantic_key__(self) -> object returning a supported value "
...
def _is_extern_function(fn) -> bool:
    ...
    dist-packages) code outside the DLKernel package. Like classes and modules,
    ...
    hashes every DLKernel source file), and recursing into them would pull
    ...
    if module == "DLKernel" or module.startswith("DLKernel."):
        return False
...
def module_locator(obj, fn) -> Optional[tuple]:
    ...
    ``GemmClassRef.__dlkernel_pool_payload__``)."""
...
# digest -> mod, for dlkernel::gemm_epi resolution (DLKernel.gemm_runtime.torch_op).
...
def install_transform_mod_payload(expected_digest: str, data: bytes) -> None:
    ...
    ``TransformARef.__dlkernel_pool_payload__``)."""
...
class GemmClassRef(NamedTuple):
    ...
    ``__dlkernel_pool_payload__``) — the payload never enters the cache key, so
    ...
    def __dlkernel_pool_payload__(self):
