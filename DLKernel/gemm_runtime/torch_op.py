...
``dlkernel::gemm_epi(str digest, Tensor[] ins, Tensor(a!)[] outs, str meta)``:
...
string, which is exactly right since they select compiled behavior. The meta
dict has to constant-fold under Dynamo, so host objects are read field by field
(see ``_config_meta``) rather than through ``__dict__``.
...
from DLKernel.gemm_config import GemmConfig, cta_tile_shape_m, default_config
...
@torch.library.custom_op("dlkernel::gemm_epi", mutates_args={"outs"}, device_types="cuda")
...
@torch.library.custom_op("dlkernel::gemm_epi_f", mutates_args=(), device_types="cuda")
def _gemm_epi_f(digest: str, ins: list[torch.Tensor], meta: str) -> list[torch.Tensor]:
    """Functional form of ``dlkernel::gemm_epi``: outputs allocated inside, real
    ...
    torch.ops.dlkernel.gemm_epi(digest, ins, outs, meta)
    return outs
...
# Field names of the config as a module-level constant, so reading them stays
# trace-safe: each getattr below folds to a Dynamo ConstantVariable.
_CONFIG_FIELDS = tuple(GemmConfig.__dataclass_fields__)


def _config_meta(cfg: Optional[GemmConfig]) -> Optional[dict]:
    """Field->value dict for ``meta`` (the op body rebuilds ``GemmConfig(**it)``).

    Reads the fields one at a time on purpose. ``cfg.__dict__`` is a *dict
    variable* to Dynamo — its items stay LazyVariableTrackers, so ``repr()`` of
    the enclosing meta dict cannot constant-fold and ``fullgraph=True`` fails
    with "Dynamo does not know how to trace builtin operator `repr`". A plain
    getattr per field yields constants (the same ones the caller's guard on the
    config would produce anyway), so the whole meta dict folds as before."""
    if cfg is None:
        return None
...
def compile_call(
    ...
    *,
    ...
    ``dlkernel::gemm_epi_f`` call (allocation inside the op, so the graph gets a
    ...
    ``dlkernel::gemm_epi`` form. Returns the same dict as eager.
    ...
    if caller_owned:
        ...
        torch.ops.dlkernel.gemm_epi(mod.semantic_digest, ins, outs, meta)
    else:
        ...
        cta_tile_m = None
        outs = torch.ops.dlkernel.gemm_epi_f(mod.semantic_digest, ins, meta)
