...
"""Functional facade ops (dlkernel::gemm, dlkernel::gemm_add, dlkernel::gemm_epi_f):
...
from DLKernel.gemm_interface import gemm, gemm_add
...
def test_gemm_add_functional_parity():
    ...
    out = torch.ops.dlkernel.gemm_add(A, B, C, bias=bias, tuned=False)
...
def test_gemm_add_functional_fp32_residual():
    ...
    out = torch.ops.dlkernel.gemm_add(A, B, C, out_dtype=torch.float32, tuned=False)
...
def test_gemm_functional_parity():
    A, B, _ = _abc()
    out = torch.ops.dlkernel.gemm(A, B, tuned=False)
...
def test_functional_fakes():
    ...
    with FakeTensorMode():
        ...
        out = torch.ops.dlkernel.gemm_add(A, B, C, out_dtype=torch.float32)
        assert out.shape == (64, 128) and out.dtype == torch.float32
        out2 = torch.ops.dlkernel.gemm(A, B)
